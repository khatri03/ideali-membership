...
import type { MembershipPaymentMethodOption, OrganizerPaymentAccountSelectionItem } from "../../../../types/membership";
...
export function MembershipPaymentAccountError({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
...
export function MembershipPaymentAccountEmpty() {
  return (
...
export function MembershipPaymentAccountStepPageContent({
  isLoading,
  isMethodsLoading,
  isSaving,
  paymentAccounts,
  ...
  onSelectPaymentAccount: (value: string) => void;
  onTogglePaymentMethod: (value: number) => void;
}) {
  return (
          ...
          <fieldset className="space-y-6" disabled={isLoading || isSaving || isMethodsLoading}>
            <label className="flex flex-col gap-3">
              ...
              <select
                ...
                onChange={(event) => onSelectPaymentAccount(event.target.value)}
              ...
              {!selectedAccount ? (
                <div className="rounded-[1.5rem] border border-dashed border-slate-200 bg-slate-50 p-6 text-sm text-slate-600">
                  ...
                  <p className="mt-2 leading-6">
                    The available methods will appear here once a payment account is chosen.
                  </p>
                </div>
              ) : isMethodsLoading ? (
                <div className="space-y-3">
                    ...
                    {paymentMethods.map((method) => {
                      ...
                      return (
                        <button
                          key={method.value}
                          type="button"
                          onClick={() => onTogglePaymentMethod(method.value)}
                          className={[
