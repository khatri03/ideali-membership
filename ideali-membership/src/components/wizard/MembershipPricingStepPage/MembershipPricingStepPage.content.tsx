...
import { getMembershipPricingDays, MEMBERSHIP_PRICING_CONTENT, MEMBERSHIP_PRICING_MONTHS, MEMBERSHIP_PRICING_OPTIONS } from "./MembershipPricingStepPage.fields";

export function MembershipPricingSkeleton() {
  return (
...
export function MembershipPricingError({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
...
export function MembershipPricingContent({
  availableDays,
  ...
  selectedPricing,
  annualPanelClassName,
}: {
  availableDays: number[];
  customPanelClassName: string;
  error: string;
  isAnnualCustomSelected: boolean;
  isAnnualSelected: boolean;
  isCustomSelected: boolean;
  isLoading: boolean;
  isSaving: boolean;
  onRetry: () => void;
  pricingInputRef: RefObject<HTMLInputElement | null>;
  ...
  annualPanelClassName: string;
}) {
  if (error) {
    return (
      <section className="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-sm">
        <MembershipPricingError message={error} onRetry={onRetry} />
      </section>
    );
  }
          ...
          <>
            ...
            <fieldset disabled={isSaving} className="space-y-4">
              <label className="block space-y-2">
                ...
                <input
                  ref={(node) => {
                    (pricingInputRef as { current: HTMLInputElement | null }).current = node;
                  }}
                  type="text"
                  inputMode="decimal"
                ...
                <div className="rounded-[1.75rem] border border-slate-200 bg-slate-50 p-4 shadow-sm">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      {isAnnualCustomSelected ? (
                        <p className="mt-1 text-sm text-slate-500">Month and date are required for the custom annual expiry.</p>
                      ) : null}
                    </div>
                  ...
                  <div className="mt-4 grid gap-4 md:grid-cols-2">
                    ...
                    <label className="space-y-2">
                      ...
                      <select
                        ...
                        className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm outline-none transition focus:border-cyan-400 focus:ring-4 focus:ring-cyan-100 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400"
                      >
                        <option value="">{selectedCustomExpiryMonth ? "Select date" : "Select a month first"}</option>
