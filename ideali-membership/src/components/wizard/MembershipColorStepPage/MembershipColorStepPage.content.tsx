...
export function MembershipColorError({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
...
export function MembershipColorStepPageContent({
  customColor,
  customColorInputRef,
  ...
  onSelectPresetColor: (value: string) => void;
          ...
          <fieldset disabled={isSaving} className="space-y-3">
            <div className="grid w-fit grid-cols-4 gap-1.5">
              {MEMBERSHIP_COLOR_SWATCHES.map((color) => {
                ...
                return (
                  <MembershipColorSwatch
                    ...
                    onSelect={() => onSelectPresetColor(color.value)}
                  />
                );
              })}
              ...
              <MembershipColorCustomTile
                ...
                onOpenPicker={onOpenCustomColorPicker}
